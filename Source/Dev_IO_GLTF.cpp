...
// void Dev::IO::Import_GLTF(const std::string& fileName, Scene& scene)
// {
...
// 					const tinygltf::Value& gltf_object_filtermask = gltf_object.Get("filtermask");
// 					object->filterMask = uint32_t(gltf_object_filtermask.Get<double>());
...
// 			if (ext_data.Has("mesh"))
// 			{
// 				const tinygltf::Value& gltf_mesh = ext_data.Get("mesh");
// 				wi::scene::MeshComponent* mesh = state.scene->meshes.GetComponent(entity);
// 				if(mesh != nullptr)
// 				{
// 					const tinygltf::Value& gltf_mesh_flags = gltf_mesh.Get("flags");
// 					mesh->_flags = uint32_t(gltf_mesh_flags.Get<double>());
// 				}
// 			}
...
// 				const tinygltf::Value& gltf_decal_mtlname = gltf_decal.Get("material");
// 				std::string gltf_decal_material_name = gltf_decal_mtlname.Get<std::string>();
...
// 				const tinygltf::Value& gltf_decal_flags = gltf_decal.Get("flags");
// 				decal._flags = uint32_t(gltf_decal_flags.Get<double>());
// 			}
...
// 				if(gltf_emitter.Has("mesh"))
// 				{
// 					const tinygltf::Value& gltf_emitter_mesh = gltf_emitter.Get("mesh");
// 					std::string src_mesh_name = gltf_emitter_mesh.Get<std::string>();
// 					wi::ecs::Entity meshID = state.scene->Entity_FindByName(src_mesh_name);
// 					wi::scene::MeshComponent* src_mesh = state.scene->meshes.GetComponent(meshID);
// 					if(src_mesh != nullptr)
// 					{
// 						emitter.meshID = meshID;
// 					}
// 				}
// 			}

// 			if (ext_data.Has("spring"))
// 			{
// 				const tinygltf::Value& gltf_spring = ext_data.Get("spring");
// 				wi::scene::SpringComponent& spring = state.scene->springs.Create(entity);

// 				const tinygltf::Value& gltf_spring_flags = gltf_spring.Get("flags");
// 				spring._flags = uint32_t(gltf_spring_flags.Get<double>());

// 				const tinygltf::Value& gltf_spring_stiffnessforce = gltf_spring.Get("stiffness_force");
// 				spring.stiffnessForce = float(gltf_spring_stiffnessforce.Get<double>());

// 				const tinygltf::Value& gltf_spring_dragforce = gltf_spring.Get("drag_force");
// 				spring.dragForce = float(gltf_spring_dragforce.Get<double>());

// 				const tinygltf::Value& gltf_spring_windforce = gltf_spring.Get("wind_force");
// 				spring.windForce = float(gltf_spring_windforce.Get<double>());

// 				const tinygltf::Value& gltf_spring_hitradius = gltf_spring.Get("hit_radius");
// 				spring.hitRadius = float(gltf_spring_hitradius.Get<double>());

// 				const tinygltf::Value& gltf_spring_gravitypower = gltf_spring.Get("gravity_power");
// 				spring.gravityPower = float(gltf_spring_gravitypower.Get<double>());

// 				const tinygltf::Value& gltf_spring_gravitydir = gltf_spring.Get("gravity_dir");
// 				spring.gravityDir.x = float(gltf_spring_gravitydir.Get(0).Get<double>());
// 				spring.gravityDir.y = float(gltf_spring_gravitydir.Get(1).Get<double>());
// 				spring.gravityDir.z = float(gltf_spring_gravitydir.Get(2).Get<double>());
// 			}

// 			if (ext_data.Has("light"))
// 			{
// 				const tinygltf::Value& gltf_light = ext_data.Get("light");
// 				wi::scene::LightComponent* light = state.scene->lights.GetComponent(state.entityMap[nodeID]);

// 				if(light != nullptr){
// 					const tinygltf::Value& gltf_light_flags = gltf_light.Get("flags");
// 					light->_flags = uint32_t(gltf_light_flags.Get<double>());

// 					const tinygltf::Value& gltf_light_cascadedistances = gltf_light.Get("cascade_distances");
// 					light->cascade_distances.clear();
// 					for(size_t i = 0; i < gltf_light_cascadedistances.ArrayLen(); ++i)
// 					{
// 						float distance = float(gltf_light_cascadedistances.Get(i).Get<double>());
// 						if(distance > 0.01f)
// 						{
// 							light->cascade_distances.push_back(distance);
// 						}
// 					}
// 				}
// 			}
...
// 				material._flags = material._flags | uint32_t(gltf_materialExtra_flags.Get<double>());
...
// 	// Retrieve all custom data here
// 	wi::unordered_map<wi::ecs::Entity, tinygltf::Value> gltf_animation_extra;
// 	wi::unordered_map< std::string, wi::unordered_map<wi::ecs::Entity, tinygltf::Value>* > gltf_extension_extdata_process_list = {
// 		{"animation_extra", &gltf_animation_extra}
// 	};
// 	auto gltf_extension_extdata_find = state.gltfModel.extensions.find(ext_name);
// 	if(gltf_extension_extdata_find != state.gltfModel.extensions.end())
// 	{
// 		tinygltf::Value gltf_extension_extdata = gltf_extension_extdata_find->second;
// 		for(std::string& process_key : gltf_extension_extdata.Keys())
// 		{
// 			if(gltf_extension_extdata_process_list.find(process_key) != gltf_extension_extdata_process_list.end())
// 			{
// 				const tinygltf::Value& key_data = gltf_extension_extdata.Get(process_key);
// 				for(size_t item_id = 0; item_id < key_data.ArrayLen(); ++item_id)
// 				{
// 					const tinygltf::Value& item_data = key_data.Get(item_id);
// 					const tinygltf::Value& entity_name = item_data.Get("name");
// 					gltf_animation_extra[state.scene->Entity_FindByName(entity_name.Get<std::string>())] = item_data;
// 				}
// 			}
// 		}
// 	}
...
// 	{
// 		tinygltf::Animation& gltf_anim = state.gltfModel.animations[animID];
// 		wi::scene::AnimationComponent& anim = state.scene->animations[animID];

// 		auto gltf_animExtra_find = gltf_animation_extra.find(state.scene->animations.GetEntity(animID));
// 		if (gltf_animExtra_find != gltf_animation_extra.end())
// 		{
// 			const tinygltf::Value& gltf_animExtra = gltf_animExtra_find->second;

// 			const tinygltf::Value& gltf_animExtra_flags = gltf_animExtra.Get("flags");
// 			anim._flags = anim._flags | uint32_t(gltf_animExtra_flags.Get<double>());
// 		}
// 	}

// 	// Data reorder for processing which disobeys hierarchy
// 	// Spring
// 	{
// 		wi::unordered_set<wi::ecs::Entity> root_list;
// 		wi::unordered_map<wi::ecs::Entity,wi::vector<wi::ecs::Entity>> hierarchy_list;
// 		for(size_t i = 0; i < state.scene->springs.GetCount(); ++i)
// 		{
// 			wi::ecs::Entity entity = state.scene->springs.GetEntity(i);
// 			wi::scene::HierarchyComponent* hierarchy = state.scene->hierarchy.GetComponent(entity);
			
// 			bool has_parent = false;
// 			if(hierarchy != nullptr)
// 			{
// 				if(hierarchy->parentID != wi::ecs::INVALID_ENTITY && (state.scene->springs.Contains(hierarchy->parentID)))
// 				{
// 					hierarchy_list[hierarchy->parentID].push_back(entity);
// 					has_parent = true;
// 				}
// 			}
...
// 			reorder_fn = [&state, &hierarchy_list, &reorder_done, &r_idx, &reorder_fn](wi::ecs::Entity target)
// 				{
// 					if (reorder_done.find(target) == reorder_done.end()){
// 						size_t idx = state.scene->springs.GetIndex(target);
// 						state.scene->springs.MoveItem(idx, r_idx);
// 						reorder_done.insert(target);
// 						r_idx.fetch_add(1);
// 					}

// 					auto find_hierarchy = hierarchy_list.find(target);
// 					if(find_hierarchy != hierarchy_list.end())
// 					{
...
// void Dev::IO::Export_GLTF(const std::string& filename, Scene& scene)
// {
...
void Import_Extension_MURAMASA_assetsmith(LoaderState& state)
{
	...
	for(size_t materialID = 0; materialID < state.gltfModel.materials.size(); ++materialID)
	{
		...
		auto ext_khr_emission = gltf_material.extensions.find("KHR_materials_emissive_strength");
		if (ext_khr_emission != gltf_material.extensions.end())
		{
			const tinygltf::Value& ext_data = ext_khr_emission->second;
			if (ext_data.Has("emissiveStrength"))
			{
				const tinygltf::Value& gltf_emissiveStrength = ext_data.Get("emissiveStrength");
				material.SetEmissiveStrength(float(gltf_emissiveStrength.Get<double>()));
			}
		}
		...
		if (ext_asm != gltf_material.extensions.end())
		{
			...
			if (ext_data.Has("material_extra"))
			{
				...
				const tinygltf::Value& gltf_materialExtra_useUserBlendMode = gltf_materialExtra.Get("use_user_blend_mode");
				bool use_user_blend_mode = bool(gltf_materialExtra_useUserBlendMode.Get<bool>());
				
				static const wi::unordered_map<std::string,wi::enums::BLENDMODE> gltf_materialExtra_userBlendMode_conversion = {
					{"BLENDMODE_OPAQUE", wi::enums::BLENDMODE_OPAQUE},
					{"BLENDMODE_ALPHA", wi::enums::BLENDMODE_ALPHA},
					{"BLENDMODE_PREMULTIPLIED", wi::enums::BLENDMODE_PREMULTIPLIED},
					{"BLENDMODE_ADDITIVE", wi::enums::BLENDMODE_ADDITIVE},
					{"BLENDMODE_MULTIPLY", wi::enums::BLENDMODE_MULTIPLY}
				};
				if(use_user_blend_mode)
				{
					const tinygltf::Value& gltf_materialExtra_userBlendMode = gltf_materialExtra.Get("user_blend_mode");
					material.userBlendMode = gltf_materialExtra_userBlendMode_conversion.find(gltf_materialExtra_userBlendMode.Get<std::string>())->second;
				}
